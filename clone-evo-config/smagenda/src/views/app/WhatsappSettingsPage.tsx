...
export function WhatsappSettingsPage() {
  ...
  const [quotaMensal, setQuotaMensal] = useState<number | null>(null)
  const [consumoMes, setConsumoMes] = useState<number | null>(null)
  const [consumoMesRef, setConsumoMesRef] = useState<string | null>(null)
  ...
  useEffect(() => {
    const run = async () => {
      ...
      if (cfg.body && typeof cfg.body === 'object') {
        const provider = (cfg.body as Record<string, unknown>).provider
        setConfigProvider(typeof provider === 'string' ? provider : null)

        const evoRaw = (cfg.body as Record<string, unknown>).evolution
        if (evoRaw && typeof evoRaw === 'object') {
          const reason = (evoRaw as Record<string, unknown>).reason
          setConfigReason(typeof reason === 'string' ? reason : null)
        }
      }
      ...
      if (quotaRes.error) {
        if (!isMissingColumnError(quotaRes.error.message)) {
          setError(quotaRes.error.message)
          setLoading(false)
          return
        }
        setQuotaMensal(null)
        setConsumoMes(null)
        setConsumoMesRef(null)
      } else {
        const row = (quotaRes.data ?? null) as unknown as {
          whatsapp_quota_mensal?: unknown
          whatsapp_consumo_mes?: unknown
          whatsapp_consumo_mes_ref?: unknown
        } | null
        const quota = typeof row?.whatsapp_quota_mensal === 'number' && Number.isFinite(row.whatsapp_quota_mensal) ? row.whatsapp_quota_mensal : null
        const consumo = typeof row?.whatsapp_consumo_mes === 'number' && Number.isFinite(row.whatsapp_consumo_mes) ? row.whatsapp_consumo_mes : null
        const ref = typeof row?.whatsapp_consumo_mes_ref === 'string' && row.whatsapp_consumo_mes_ref.trim() ? row.whatsapp_consumo_mes_ref.trim() : null
        setQuotaMensal(quota)
        setConsumoMes(consumo)
        setConsumoMesRef(ref)
      }
  ...
  const connect = async () => {
    ...
    if (!configurado) {
      setError('WhatsApp ainda não está configurado. Solicite ao suporte.')
      return
    }
    setConnecting(true)
    ...
    if (!res.ok) {
      ...
      if (typeof res.body === 'object' && res.body !== null && (res.body as Record<string, unknown>).error === 'whatsapp_not_configured') {
        setError('WhatsApp ainda não está configurado. Configure seu gateway (Evolution) ou solicite ao suporte.')
        setConnecting(false)
        return
      }
  ...
  const quotaLabel = (() => {
    if (quotaMensal === null || consumoMes === null) return null
    const refPart = consumoMesRef ? ` (${consumoMesRef})` : ''
    return `${consumoMes}/${quotaMensal}${refPart}`
  })()
              ...
              {loading ? (
                ...
                <div className="space-y-2">
                  ...
                  <div className="text-sm text-slate-700">
                    Modo: {configurado ? 'Global' : '—'}{configProvider ? ` (${configProvider})` : ''}
                    {configReason ? ` • ${configReason}` : ''}
                  </div>
                  <div className="text-sm text-slate-700">Conexão: {connectionLabel}</div>
                  {quotaLabel ? <div className="text-sm text-slate-700">Consumo do mês: {quotaLabel}</div> : null}
        ...
        >
          <Card>
            <div className="p-6 space-y-4">
              ...
              <Input label="Número (com DDD)" value={testNumber} onChange={(e: ChangeEvent<HTMLInputElement>) => setTestNumber(e.target.value)} placeholder="11 99999-9999" />
              <div className="space-y-2">
                ...
                <textarea
                  ...
                  value={testText}
                  onChange={(e: ChangeEvent<HTMLTextAreaElement>) => setTestText(e.target.value)}
