...
function isMissingRelationshipError(message: string) {
  return message.toLowerCase().includes('could not find a relationship between')
}
...
Deno.serve(async (req) => {
  ...
  const saFirst = await adminClient
    .from('super_admin')
    .select('id,whatsapp_api_url,whatsapp_api_key,whatsapp_meta_access_token,whatsapp_meta_phone_number_id')
    .limit(1)
    .maybeSingle()

  const saSecond =
    saFirst.error && isMissingColumnError(saFirst.error.message)
      ? await adminClient.from('super_admin').select('id,whatsapp_api_url,whatsapp_api_key').limit(1).maybeSingle()
      : null

  const used = saSecond ?? saFirst

  if (used.error) {
    if (isMissingColumnError(used.error.message) || isMissingTableError(used.error.message)) {
      return jsonResponse(400, { error: 'schema_incomplete', message: used.error.message })
    }
    return jsonResponse(400, { error: 'load_super_admin_failed', message: used.error.message })
  }

  const sa = (used.data ?? null) as unknown as SuperAdminConfigRow | null
  ...
  const globalEvolutionCfg = globalApiUrl && globalApiKey ? { apiUrl: globalApiUrl, apiKey: globalApiKey } : null
  const globalMetaCfg = META_ENABLED && metaAccessToken && metaPhoneNumberId ? { accessToken: metaAccessToken, phoneNumberId: metaPhoneNumberId } : null

  async function tryResetWhatsappUsageIfNeeded(usuarioId: string) {
    const { error } = await adminClient.rpc('whatsapp_reset_usage_if_needed', { p_usuario_id: usuarioId })
  ...
  async function loadWhatsappQuotaAndUsage(usuarioId: string) {
  ...
  async function tryRegisterOutboundMessage(args: { usuarioId: string; messageId: string; kind: string; category: string; raw: unknown }) {
    const { error } = await adminClient.rpc('whatsapp_register_outbound', {
  ...
  async function sendTextWithProvider(args: {
    usuarioId: string
    instanceName: string
    phoneRaw: string
    text: string
    kind: string
    category: string
    evolution?: { apiUrl: string; apiKey: string } | null
  }) {
    const userEvolutionCfg = args.evolution ?? null
    if (userEvolutionCfg) {
      const sendRes = await evolutionSendTextWithFallback({
        apiUrl: userEvolutionCfg.apiUrl,
        apiKey: userEvolutionCfg.apiKey,
        instanceName: args.instanceName,
        phoneRaw: args.phoneRaw,
        text: args.text,
      })
      if (!sendRes.ok) return { ok: false as const, error: 'evolution_error' as const, status: sendRes.status || 502, details: sendRes.body, attempts: sendRes.attempts }
      return { ok: true as const, provider: 'evolution' as const }
    }

    if (globalMetaCfg) {
      const to = normalizeMetaRecipient(args.phoneRaw)
      if (!to) return { ok: false as const, error: 'invalid_phone' as const, status: 400 }

      await tryResetWhatsappUsageIfNeeded(args.usuarioId)
      const quotaRes = await loadWhatsappQuotaAndUsage(args.usuarioId)
      if (quotaRes.ok && isWhatsappQuotaExceeded({ quota: quotaRes.quota, consumo: quotaRes.consumo })) {
        return {
          ok: false as const,
          error: 'whatsapp_quota_exceeded' as const,
          status: 402,
      ...
      const sendRes = await metaSendText({ accessToken: globalMetaCfg.accessToken, phoneNumberId: globalMetaCfg.phoneNumberId, to, text: args.text })
      if (!sendRes.ok) return { ok: false as const, error: 'meta_error' as const, status: sendRes.status || 502, details: sendRes.body }

      const messageId = metaExtractMessageId(sendRes.body)
      if (messageId) {
        await tryRegisterOutboundMessage({ usuarioId: args.usuarioId, messageId, kind: args.kind, category: args.category, raw: sendRes.body })
      }

      return { ok: true as const, provider: 'meta' as const, messageId }
    }

    if (globalEvolutionCfg) {
      const sendRes = await evolutionSendTextWithFallback({
        apiUrl: globalEvolutionCfg.apiUrl,
        apiKey: globalEvolutionCfg.apiKey,
        instanceName: args.instanceName,
        phoneRaw: args.phoneRaw,
        text: args.text,
      })
    ...
    return { ok: false as const, error: 'not_configured' as const, status: 200 }
  }
  ...
  if (action === 'billing_status_changed') {
    ...
    const { data: uRaw, error: uErr } = await adminClient
      .from('usuarios')
      .select('id,whatsapp_instance_name,slug,nome_negocio,telefone,whatsapp_api_url,whatsapp_api_key,whatsapp_habilitado,status_pagamento,data_vencimento,plano,ativo')
      .eq('id', usuarioId)
      .maybeSingle()
    ...
    const uApiUrl = typeof u.whatsapp_api_url === 'string' ? u.whatsapp_api_url.trim() : ''
    const uApiKey = typeof u.whatsapp_api_key === 'string' ? u.whatsapp_api_key.trim() : ''
    const userEvolutionCfg = uApiUrl && uApiKey ? { apiUrl: uApiUrl, apiKey: uApiKey } : null
    ...
    const sendRes = await sendTextWithProvider({ usuarioId: u.id, instanceName, phoneRaw: u.telefone ?? '', text, kind: 'billing_status', category: 'utility', evolution: userEvolutionCfg })
    if (!sendRes.ok) {
      if (sendRes.error === 'whatsapp_quota_exceeded') {
        return jsonResponse(402, {
          error: 'whatsapp_quota_exceeded',
          quota_mensal: sendRes.quota_mensal,
          consumo_mes: sendRes.consumo_mes,
          consumo_mes_ref: sendRes.consumo_mes_ref,
        })
      }
      return jsonResponse(502, { ok: false, error: sendRes.error, details: 'details' in sendRes ? sendRes.details : null, attempts: 'attempts' in sendRes ? sendRes.attempts : null })
    }
    return jsonResponse(200, { ok: true, sent: true, usuario_id: u.id, status_pagamento: status, provider: sendRes.provider })
  }
  ...
  if (action === 'billing_daily') {
    ...
    for (const row of usuarios ?? []) {
      ...
      const sendRes = await sendTextWithProvider({ usuarioId: u.id, instanceName, phoneRaw: phone, text, kind: `billing_daily:${stage.kind}`, category: 'utility' })
      if (!sendRes.ok) {
        if (sendRes.error === 'whatsapp_quota_exceeded') {
          skipped.push({ usuario_id: u.id, reason: 'whatsapp_quota_exceeded' })
          continue
        }
        skipped.push({ usuario_id: u.id, reason: sendRes.error })
        continue
      }
  ...
  if (action === 'funcionario_novo_agendamento') {
    ...
    const sendRes = await sendTextWithProvider({
      usuarioId: uRow.id,
      instanceName,
      phoneRaw: funcionarioTelefone,
      text,
      kind: 'funcionario_novo_agendamento',
      category: 'utility',
    })
    ...
    return jsonResponse(200, { ok: true, sent: true, usuario_id: uRow.id, agendamento_id: agendamentoId, provider: sendRes.provider })
  }
  ...
  if (agendamentoId) {
    ...
    const sendRes = await sendTextWithProvider({ usuarioId: uRow.id, instanceName, phoneRaw, text, kind: 'confirmacao', category: 'utility' })
    if (!sendRes.ok) {
      if (sendRes.error === 'whatsapp_quota_exceeded') {
        return jsonResponse(402, {
          error: 'whatsapp_quota_exceeded',
    ...
    return jsonResponse(200, { ok: true, sent: true, usuario_id: uRow.id, agendamento_id: agRow.id, provider: sendRes.provider })
  }
    ...
    if (uRow.enviar_lembrete === true) {
      ...
      if (agErr) {
        ...
        for (const ag of ags ?? []) {
          ...
          const sendRes = await sendTextWithProvider({ usuarioId: uRow.id, instanceName, phoneRaw, text, kind: 'lembrete', category: 'utility' })
          if (!sendRes.ok) {
            if (sendRes.error === 'whatsapp_quota_exceeded') {
              reminderSkipped++
              continue
